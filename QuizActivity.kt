...
class QuizActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imageView = findViewById<ImageView>(R.id.StateOfMatter)
        imageView.setOnClickListener {
            val dialogBuilder = AlertDialog.Builder(this, R.style.CustomAlertDialogStyle)
            val dialog= dialogBuilder

                .setTitle("How would you describe yourself?")
                .setMessage("There are three states of matter at standard temperature and" +
                        " pressure: gas,liquid and solid!")
        ...
        button1.setOnClickListener {
            ...
            editor.putInt("BUTTON_ID1", R.id.button)
            editor.apply()
        }
        button2.setOnClickListener {
            val sharedPref = getSharedPreferences("MY_PREFS_NAME", Context.MODE_PRIVATE)
            val editor = sharedPref.edit()
            editor.putInt("BUTTON_ID1", R.id.button2)
            editor.apply()
        }
        button3.setOnClickListener {
            val sharedPref = getSharedPreferences("MY_PREFS_NAME", Context.MODE_PRIVATE)
            val editor = sharedPref.edit()
            editor.putInt("BUTTON_ID1", R.id.button3)
            editor.apply()
        }
